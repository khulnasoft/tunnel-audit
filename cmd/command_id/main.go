...
func GetCommandIDRange() ([]bool, error) {
	...
	entries, err := tunnel_audit.EmbeddedK8sCommandsFileSystem.ReadDir(commandPath)
	if err != nil {
		panic(err)
	}
	for _, entry := range entries {
		...
		fContent, err := tunnel_audit.EmbeddedK8sCommandsFileSystem.ReadFile(filepath.Join(commandPath, entry.Name()))
		if err != nil {
			return nil, err
		}
		var fileCommand any
